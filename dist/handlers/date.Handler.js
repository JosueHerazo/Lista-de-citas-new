...
const getProducts = async (req, res) => {
    try {
        const ListDate = await List_models_1.default.findAll({
...
const createProduct = async (req, res) => {
    try {
        ...
        const dateslist = await List_models_1.default.create(req.body);
...
const getProductById = async (req, res) => {
    try {
        const { id } = req.params;
        const ListDate = await List_models_1.default.findByPk(id);
        if (!ListDate)
...
const getBarberos = async (req, res) => {
    try {
        const config = await List_models_1.default.findOne({ where: { service: '__barberos__' } });
        if (!config) {
...
const saveBarberos = async (req, res) => {
    try {
        ...
        const existing = await List_models_1.default.findOne({ where: { service: '__barberos__' } });
        if (existing) {
